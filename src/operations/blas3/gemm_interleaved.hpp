...
class Gemm<input_t, output_t, /* DoubleBuffer = */ false, /* NbcA = */ false,
           ...
           static_cast<int>(gemm_vectorization_t::full), /* VectorSize = */ 1,
 ...
 public:
  ...
  SYCL_BLAS_INLINE index_t get_workgroup_cluster() const noexcept {
    return ((m_ - 1) / (item_rows * wg_rows) + 1) *
           ((n_ - 1) / (item_cols * wg_cols) + 1);
  }
   ...
   * operations is available per work group. The number 4 is used based on
   * empirical research.
   */
  ...
  SYCL_BLAS_INLINE void eval(cl::sycl::nd_item<1> id) noexcept {
    ...
    const index_t number_of_block_per_batch =
        ((batch_size_ / item_batchs - 1) / block_batchs) + 1;
    ...
    if (is_internal_block) {
      compute_panel<false>(boundary_check, single_a_stride, single_b_stride,
                           mb_start, m_start, n_start, A, B, C
#ifdef ARM_GPU
                           ,
                           id
#endif
      ...
      compute_panel<true>(boundary_check, single_a_stride, single_b_stride,
                          mb_start, m_start, n_start, A, B, C
#ifdef ARM_GPU
                          ,
                          id
  ...
  SYCL_BLAS_INLINE void compute_panel(check_t boundary_check,
                                      index_t single_a_stride,
                                      index_t single_b_stride, index_t mb_start,
                                      index_t m_start, index_t n_start, ptr_t A,
                                      ptr_t B, ptr_t C
#ifdef ARM_GPU
                                      ,
  ...
  template <index_t item_size, bool need_check_boundary, typename check_t,
            typename ptr_t>
  SYCL_BLAS_INLINE void load(check_t boundary_check, index_t index_start,
                             index_t mb_start, index_t dim_size,
...
#pragma unroll
  ...
  template <bool need_check_boundary, typename check_t, typename ptr_t>
  SYCL_BLAS_INLINE void store(check_t boundary_check, index_t m_start,
          ...
          if (b_range) {
            ...
            for (int p = 0; p < item_batchs; ++p) {
              auto is_in = do_check<need_check_boundary>(
                  boundary_check(mb_start + p, batch_size_));
              if (is_in) {
                output[p] = reinterpret_cast<value_t *>(reg_res)[p];
              }
            }
          }
        }
            ...
            *reg_res *= beta_;
            ...
            for (int p = 0; p < item_batchs; ++p) {
              auto is_in = do_check<need_check_boundary>(
                  boundary_check(mb_start + p, batch_size_));
              reinterpret_cast<value_t *>(reg_res)[p] =
                  is_in ? (output[p] * beta_) : value_t(0);
            }
          }
        }
        ++reg_res;
